...
int main(int argc, char* argv[]) {
  ...
  // 击打相关参数
  const float target_stable_dist = 0.1f;    
  const int stable_frame_threshold = 3;     
  const std::chrono::seconds fire_interval = 1s;  
  ...
  std::chrono::steady_clock::time_point last_fire_time = std::chrono::steady_clock::time_point::min(); 
  json plot_data;
  int stable_frame_count = 0;                
  Eigen::Vector3d last_target_pos;           
  bool is_target_stable = false;             
  ...
  while (!exiter.exit()) {
    ...
    if (target_armor_ptr != nullptr && is_auto_aim_mode) {
      ...
      //tools::Trajectory trajectory(bullet_speed, distance, target_pos.z());
